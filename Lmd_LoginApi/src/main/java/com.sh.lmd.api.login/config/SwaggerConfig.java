package com.sh.lmd.api.login.config;
...
public class SwaggerConfig {
    ...
    public ApiInfo createA(){
        ApiInfo info=new ApiInfoBuilder().title("统一登录鉴权中心-数据接口平台").
                contact( new Contact("冯自祥","http://www.mobiletrain.org","123456@163.com")).
    ...
    public Docket createDocket(){
       Docket docket=new Docket(DocumentationType.SWAGGER_2).apiInfo(createA()).select().apis(
               RequestHandlerSelectors.basePackage("com.sh.lmd.api.login.controller")).build();
       return docket;
    }
}